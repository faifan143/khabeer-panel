...
export default function RatingsPage() {
  ...
  if (isLoading) {
    return (
      <PermissionRoute requiredPermissions={[PERMISSIONS.RATINGS_VIEW]}>
        <AdminLayout>
          <div className="flex items-center justify-center h-32">
            <div className="text-center">
              <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary mx-auto mb-2"></div>
              <div className="text-sm text-muted-foreground">
                {t("ratings.loadingRatings")}
              </div>
            </div>
          </div>
        </AdminLayout>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader className="flex flex-col lg:flex-row items-center justify-between">
              <div>
                <CardTitle>
                  {t("ratings.allRatings", { count: filteredRatings.length })}
                </CardTitle>
                              ...
                              <DialogContent className="max-w-2xl ">
                                ...
                                {selectedRating && (
                                  <div className="space-y-6">
                                        ...
                                        <Avatar className="h-12 w-12">
                                          <AvatarImage
                                            src={
                                              ...
                                              selectedRating.user?.image
                                            }
                                            alt={selectedRating.user?.name}
                                          />
                                          <AvatarFallback>
                                            {getInitials(
                                              selectedRating.user?.name || "U"
                                            )}
                                          </AvatarFallback>
                                        </Avatar>
                                        <div>
                                    ...
                                    {selectedRating.order && (
                                      <div className="bg-gray-50 p-4 rounded-lg">
                                        <h4 className="font-medium mb-2">
                                          {t("ratings.serviceInformation")}
                                        </h4>
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                          <div>
                                            <span className="text-muted-foreground">
                                              {t("ratings.providerLabel")}
                                            </span>
                                            <div className="font-medium">
                                              {selectedRating.provider?.name ||
                                                t("ratings.unknown")}
                                            </div>
                                          </div>
                                          <div>
                                          ...
                                          <div>
                                            <span className="text-muted-foreground">
                                              {t("ratings.orderIdLabel")}
                                            </span>
                                            <div className="font-medium">
                                              #{selectedRating.order.id}
                                            </div>
                                          </div>
                                          <div>
                                            <span className="text-muted-foreground">
                                              {t("ratings.ratingDateLabel")}
                                            </span>
                                            <div className="font-medium">
                                              {formatDate(
                                                selectedRating.ratingDate
                                              )}
                                            </div>
                                          </div>
                                        </div>
                                    ...
                                    <div>
                                      <h4 className="font-medium mb-2">
                                        {t("ratings.customerComment")}
                                      </h4>
                                      <div className="bg-gray-50 p-4 rounded-lg">
                                        <p className="text-sm">
                                          {selectedRating.comment ||
                                            t("ratings.noCommentProvided")}
                                        </p>
                                      </div>
                                    </div>
                                  </div>
