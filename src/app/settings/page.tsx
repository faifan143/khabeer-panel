...
export default function SettingsPage() {
  ...
  const availablePermissions = [
    {
      id: "dashboard",
      label: t("settings.dashboardAccess"),
      description: t("settings.viewDashboardStatistics"),
    },
    {
      id: "orders",
      label: t("settings.ordersManagement"),
      description: t("settings.manageAndViewOrders"),
    },
    {
      id: "users",
      label: t("settings.usersManagement"),
      description: t("settings.manageCustomerAccounts"),
    },
    {
      id: "providers",
      label: t("settings.providerManagement"),
    ...
    {
      id: "ratings",
      label: t("settings.ratingsReviews"),
      description: t("settings.manageRatingsReviews"),
    },
    {
      id: "notifications",
      label: t("settings.notifications"),
      description: t("settings.sendSystemNotifications"),
    },
    {
      id: "settings",
  ...
  const [editingBannerIndex, setEditingBannerIndex] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    if (systemSettings) {
      ...
      const socialSetting = systemSettings.social?.find(
        (s) => s.key === "social_links"
      );
      if (socialSetting) {
        try {
      ...
      // Load support links
      const supportSetting = systemSettings.support?.find(
        (s) => s.key === "whatsapp_support"
      );
      ...
      const privacyEnSetting = legalSettings.find(
        (s) => s.key === "privacy_en"
      );
      const privacyArSetting = legalSettings.find(
        (s) => s.key === "privacy_ar"
      );
  ...
  const handleLegalFileSelect = (
    event: React.ChangeEvent<HTMLInputElement>,
    type: "termsEn" | "termsAr" | "privacyEn" | "privacyAr"
  ) => {
    const file = event.target.files?.[0];
    if (file) {
  ...
  const handleSaveTerms = async () => {
    ...
    try {
      ...
      const uploadResult = await uploadLegalDocumentsMutation.mutateAsync([
        termsEn,
        termsAr,
      ]);
  ...
  const handleAddSubAdmin = async () => {
    if (
      ...
      newSubAdmin.permissions.length === 0
    ) {
  ...
  const handleSaveAdBanner = async () => {
    ...
    try {
      const bannerData: any = {
        ...
        providerId: newBanner.providerId
          ? parseInt(newBanner.providerId)
          : undefined,
                      ...
                      <div className="space-y-3 mt-2">
                        {termsEn ? (
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="border-2 border-dashed rounded-lg p-4 text-center">
                              ...
                              <p className="text-sm text-gray-600 mb-2">
                                {t("settings.uploadNewFileToReplace")}
                              </p>
                              <Button
                                variant="outline"
                                size="sm"
                          ...
                          <div className="border-2 border-dashed rounded-lg p-6 text-center">
                            ...
                            <p className="text-sm text-gray-600 mb-2">
                              {t("settings.uploadTermsConditions")} (
                              {t("settings.englishVersion")})
                            </p>
                            <Button
                              variant="outline"
                      ...
                      <div className="space-y-3 mt-2">
                        {privacyEn ? (
                          ...
                          <div className="border-2 border-dashed rounded-lg p-6 text-center">
                            <Upload className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                            <p className="text-sm text-gray-600 mb-2">
                              {t("settings.uploadPrivacyPolicy")} (
                              {t("settings.englishVersion")})
                            </p>
                            <Button
                              variant="outline"
                        ...
                        <input
                          ...
                          accept=".pdf,.doc,.docx,.txt"
                          onChange={(e) =>
                            handleLegalFileSelect(e, "privacyEn")
                          }
                          className="hidden"
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-3 mt-2">
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="border-2 border-dashed rounded-lg p-4 text-center">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  privacyArInputRef.current?.click()
                                }
                              >
                                {t("settings.chooseNewFile")}
                              </Button>
                        ...
                        <input
                          ...
                          accept=".pdf,.doc,.docx,.txt"
                          onChange={(e) =>
                            handleLegalFileSelect(e, "privacyAr")
                          }
                          className="hidden"
                        />
                      </div>
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Dialog
                      open={isAddSubAdminOpen}
                      onOpenChange={setIsAddSubAdminOpen}
                    >
                      ...
                      <DialogContent className="max-w-2xl">
                        <DialogHeader>
                          <DialogTitle>
                            {t("settings.addNewSubAdministrator")}
                          </DialogTitle>
                          <DialogDescription>
                            ...
                            <div className="space-y-2 mt-2 max-h-60 overflow-y-auto border rounded-md p-3">
                              {availablePermissions.map((permission) => (
                                ...
                                >
                                  <Checkbox
                                    id={permission.id}
                                    checked={newSubAdmin.permissions.includes(
                                      permission.id
                                    )}
                                    onCheckedChange={() =>
                        ...
                        subAdmins.map((admin) => (
                          <TableRow key={admin.id}>
                            ...
                            <TableCell>
                              <Badge
                                variant={
                                  admin.isActive ? "default" : "secondary"
                                }
                              >
                                {admin.isActive
                                  ? t("settings.active")
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setNewBanner((prev) => ({
                            ...prev,
                            linkType: value as "external" | "provider",
                          }))
                        }
                      >
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      <Button onClick={handleSaveAdBanner}>
                        <Save className="h-4 w-4 mr-2" />
                        {editingBannerIndex !== null
                          ? t("settings.updateBanner")
                          : t("settings.createBanner")}
                      </Button>
                    </div>
                  </div>
                </DialogContent>
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          setNewVersion((prev) => ({
                            ...prev,
                            requiresUpdate: checked as boolean,
                          }))
                        }
                      />
                      <Label
                        htmlFor="version-requires-update"
                        className="text-sm font-medium"
                      >
