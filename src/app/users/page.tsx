...
export default function UsersManagementPage() {
  ...
  const { data: userReport, isLoading: reportLoading } = useUserReport(
    startDate || undefined,
    endDate || undefined
  );
  ...
  const filteredUsers =
    userReport?.filter((user: UserReport) => {
      const matchesSearch =
        ...
        (user.state &&
          getLocalizedState(user.state, language)
            .toLowerCase()
  ...
  return (
    <PermissionRoute requiredPermissions={[PERMISSIONS.USERS_VIEW]}>
      <AdminLayout>
        <div className="space-y-6">
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {reportLoading || !userReport?.length
                    ...
                    : Math.round(
                        userReport.reduce(
                          (sum: number, user: UserReport) =>
                            sum + user.completedOrders,
                          0
                        ) / userReport.length
                      )}
                </div>
          ...
          <Card>
            <CardHeader className="flex items-center justify-between">
              <div>
                <CardTitle>
                  {t("users.usersSection", { count: filteredUsers.length })}
                </CardTitle>
                <CardDescription>
                      ...
                      {filteredUsers.map((user: UserReport) => (
                        <TableRow key={user.userId}>
                          ...
                          <TableCell>
                            <div
                              className={`flex items-center ${
                                isRTL ? "space-x-reverse gap-1" : "gap-1"
                              }`}
                            >
                          ...
                          <TableCell>
                            ...
                            >
                              ...
                              <span className="text-sm font-medium">
                                {formatCurrency(user.totalSpent, i18n.language)}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell>
