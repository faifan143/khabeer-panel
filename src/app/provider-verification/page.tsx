...
import {
  useAdminActivateProvider,
  useAdminDeactivateProvider,
  useAdminPendingJoinRequests,
  useAdminApproveJoinRequest,
  useAdminRejectJoinRequest,
  useAdminProviders,
  ...
  <Card className="group hover:shadow-md transition-all duration-300 border-0 bg-gradient-to-br from-white to-gray-50/50">
    <CardContent className="px-4">
      ...
      >
        ...
        <div
          className={`w-8 h-8 rounded-md flex items-center justify-center ${color} group-hover:scale-105 transition-transform duration-300`}
        >
...
export default function ProviderVerificationPage() {
  ...
  const { data: verifiedProviders, isLoading: verifiedProvidersLoading } =
    useAdminProviders();
  ...
  if (joinRequestsResponse) {
    if (Array.isArray(joinRequestsResponse)) {
      ...
      Array.isArray(joinRequestsResponse.data)
    ) {
      ...
      joinRequestsResponse.joinRequests &&
      Array.isArray(joinRequestsResponse.joinRequests)
    ) {
  ...
  if (
    ...
    console.log(
      "No join requests found, creating from unverified providers:",
      unverifiedProviders
    );
    ...
    console.log(
      "Created join requests from unverified providers:",
      joinRequests
    );
  }
  ...
  const providerStats = useMemo(() => {
    if (
      ...
      !Array.isArray(verifiedProviders.providers)
    ) {
      return {
        total: 0,
        active: 0,
    ...
    const totalIncome = providers.reduce((sum, provider) => {
      ...
      const providerIncome =
        provider.orders?.reduce(
  ...
  const getFilteredProviders = (providerList: AdminProvider[]) => {
    let filtered = providerList.filter(
      (provider) =>
        ...
        getLocalizedState(provider.state, language)
          .toLowerCase()
  ...
  const currentProviders = getFilteredProviders(
    verifiedProviders?.providers || []
  );

  // Calculate provider income from actual data
  const calculateProviderIncome = (providerId: number) => {
    const provider = verifiedProviders?.providers?.find(
      (p) => p.id === providerId
    );
    if (!provider) {
      return {
    ...
    const totalIncome =
      provider.orders?.reduce((sum, order) => sum + order.totalAmount, 0) || 0;
    ...
    const providerAmount =
      provider.orders?.reduce((sum, order) => sum + order.providerAmount, 0) ||
      0;

    // Calculate offer discounts from provider services and offers
    const offerDiscounts =
      provider.offers?.reduce(
        (sum, offer) => sum + (offer.originalPrice - offer.offerPrice),
        0
      ) || 0;
  ...
  const handleDeactivateProvider = async (providerId: number) => {
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : t("providers.failedToDeactivate");
      toast.error(errorMessage);
    }
  };
  ...
  const handleApproveJoinRequest = async (requestId: number) => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : t("providers.failedToApprove");
      toast.error(errorMessage);
    }
  };
  ...
  const handleRejectJoinRequest = async (requestId: number) => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : t("providers.failedToReject");
      toast.error(errorMessage);
    }
  };
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-4">
            ...
            <StatCard
              ...
              value={`${providerStats.totalIncome} ${
                i18n.language === "ar" ? "ر.ع." : "OMR"
              }`}
              icon={DollarSign}
                ...
                {activeTab === "verified" && (
                  <Select value={statusFilter} onValueChange={setStatusFilter}>
                    <SelectTrigger className="w-40">
                      <SelectValue
                        placeholder={t("providers.filterByStatus")}
                      />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">
                        {t("providers.allStatus")}
                      </SelectItem>
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {currentProviders.map((provider) => {
                    ...
                    return (
                      <Card
                        key={provider.id}
                        className="group hover:shadow-xl transition-all duration-300 border-0 bg-gradient-to-br from-white to-gray-50/50 hover:from-blue-50/50 hover:to-indigo-50/50"
                      >
                        <CardContent className="p-6">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-muted-foreground mb-3">
                                {provider.description ||
                                  t("providers.professionalProvider")}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center space-x-2 text-sm">
                              ...
                              <span className="text-muted-foreground">
                                {t("providers.offerDiscounts")}:{" "}
                                {renderCurrency(income.offerDiscounts)}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-end space-x-2 mt-4 pt-4 border-t border-gray-100 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            {!provider.isActive ? (
                              <RTLConfirmationDialog
                                ...
                                description={t(
                                  "providers.activateProviderConfirm"
                              ...
                              <RTLConfirmationDialog
                                ...
                                description={t(
                                  "providers.deactivateProviderConfirm"
                                ).replace("{name}", provider.name)}
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-gray-50">
                        ...
                        >
                          {t("providers.tableHeaders.provider")}
                        </TableHead>
                        <TableHead
                          className={`font-semibold ${
                            isRTL ? "text-right" : "text-left"
                        ...
                        >
                          {t("providers.tableHeaders.contactLocation")}
                        </TableHead>
                        <TableHead
                          className={`font-semibold ${
                            isRTL ? "text-right" : "text-left"
                        ...
                        >
                          {t("providers.tableHeaders.categoriesServices")}
                        </TableHead>
                        <TableHead
                          className={`font-semibold ${
                            isRTL ? "text-right" : "text-left"
                        ...
                        >
                          {t("providers.tableHeaders.incomingOrders")}
                        </TableHead>
                        <TableHead
                          className={`font-semibold ${
                            isRTL ? "text-right" : "text-left"
                        ...
                        >
                          {t("providers.tableHeaders.detailedIncome")}
                        </TableHead>
                        <TableHead
                          className={`font-semibold ${
                            isRTL ? "text-right" : "text-left"
                        ...
                        >
                          {t("providers.tableHeaders.commissionFetched")}
                        </TableHead>
                        <TableHead
                          className={`font-semibold ${
                            isRTL ? "text-right" : "text-left"
                        ...
                        >
                          {t("providers.tableHeaders.documents")}
                        </TableHead>
                        <TableHead
                          className={`font-semibold ${
                            isRTL ? "text-right" : "text-left"
                          ...
                          >
                            ...
                            <TableCell>
                              <Button
                                ...
                                className={`h-8 px-3 hover:bg-green-100 ${
                                  isRTL
                                    ? "text-right justify-end"
                                    : "text-left justify-start"
                                } cursor-pointer`}
                            ...
                            <TableCell>
                              <div className="flex items-center space-x-2">
                                ...
                                <span className="text-sm font-semibold text-orange-700">
                                  {renderCurrency(
                                    (provider as any).totalCommission || 0
                                  )}
                                </span>
                              </div>
                            </TableCell>
                ...
                <Card className="border-0 shadow-lg">
                  <CardContent className="p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">
                      {t("providers.noVerifiedProvidersFound")}
                    </h3>
                    <p className="text-muted-foreground">
                      {searchTerm
                        ? t("providers.noProvidersMatchSearch").replace(
                            "{searchTerm}",
                            searchTerm
                          )
                        : t("providers.noVerifiedProvidersAvailable")}
                    </p>
                  </CardContent>
                </Card>
              )}
              ...
              {joinRequestsLoading && (
                <Card className="border-0 shadow-lg">
                  <CardContent className="p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">
                      {t("providers.loadingJoinRequests")}
                    </h3>
                    <p className="text-muted-foreground">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-gray-50">
                        ...
                        >
                          {t("providers.tableHeaders.contactInfo")}
                        </TableHead>
                        <TableHead
                          className={`font-semibold ${
                            isRTL ? "text-right" : "text-left"
                        ...
                        >
                          {t("providers.tableHeaders.state")}
                        </TableHead>
                        <TableHead
                          className={`font-semibold ${
                            isRTL ? "text-right" : "text-left"
                        ...
                        >
                          {t("providers.tableHeaders.requestDate")}
                        </TableHead>
                        <TableHead
                          className={`font-semibold ${
                            isRTL ? "text-right" : "text-left"
                          ...
                          <TableCell>
                            ...
                            >
                              ...
                              <div className="space-y-1">
                                <div className="font-medium text-gray-900">
                                  {request.provider?.name ||
                                    t("providers.unknownProvider")}
                                </div>
                                <div className="text-sm text-muted-foreground">
                                  {request.provider?.description ||
                                    t("providers.noDescriptionAvailable")}
                                </div>
                              </div>
                            </div>
                          </TableCell>
                          ...
                          >
                            <div className={`flex items-center gap-2`}>
                              <RTLConfirmationDialog
                                ...
                                description={t(
                                  "providers.approveJoinRequestConfirm"
                                ).replace(
                                  "{name}",
                                  request.provider?.name ||
                                    t("providers.thisProvider")
                                )}
                              ...
                              <RTLConfirmationDialog
                                ...
                                description={t(
                                  "providers.rejectJoinRequestConfirm"
                                ).replace(
                                  "{name}",
                                  request.provider?.name ||
                                    t("providers.thisProvider")
              ...
              {joinRequests.length === 0 && !joinRequestsLoading && (
                <Card className="border-0 shadow-lg">
                  <CardContent className="p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">
                      {t("providers.noPendingJoinRequests")}
                    </h3>
                    <p className="text-muted-foreground">
                    ...
                    {unverifiedProviders && unverifiedProviders.length > 0 && (
                      <p className="text-sm text-blue-600 mt-2">
                        {t("providers.foundUnverifiedProviders").replace(
                          "{count}",
                          unverifiedProviders.length.toString()
                        )}
                      </p>
                    )}
                  </CardContent>
                    ...
                    <div className="flex items-center space-x-2 text-red-700">
                      ...
                      <div>
                        <h4 className="font-medium">
                          {t("providers.errorLoadingJoinRequests")}
                        </h4>
                        <p className="text-sm text-red-600">
                          {joinRequestsError instanceof Error
                            ? joinRequestsError.message
                            : t("providers.failedToLoadJoinRequests")}
                        </p>
                      </div>
                    </div>
                  </CardContent>
              ...
              {selectedProvider && (
                <div className="space-y-6 overflow-y-auto max-h-[calc(90vh-140px)] pr-2">
                  ...
                  <div className="space-y-4">
                    <div>
                      <Label className="text-sm font-medium">
                        {t("providers.providerInformation")}
                      </Label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <Label className="text-sm font-medium">
                          {t("providers.contactInformation")}
                        </Label>
                        ...
                        <div className="mt-1 p-3 bg-gray-50 rounded-lg space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-muted-foreground">
                              {t("providers.accountLabels.active")}
                            </span>
                            <Badge
                              variant={
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-muted-foreground">
                              {t("providers.accountLabels.verified")}
                            </span>
                            <Badge
                              variant={
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-muted-foreground">
                              {t("providers.accountLabels.joined")}
                            </span>
                            <span className="text-sm">
                    ...
                    {selectedProvider?.providerServices &&
                      selectedProvider.providerServices.length > 0 && (
                        <div>
                          <Label className="text-sm font-medium">
                            {t("providers.servicesOffered")}
                          </Label>
                          <div className="mt-1 space-y-2">
                            {selectedProvider.providerServices.map(
                              (service, index: number) => {
                                // Find matching offer for this service based on price
                                const matchingOffer =
                                  selectedProvider.offers?.find(
                                    (offer) =>
                                      offer.originalPrice === service.price
                                  );
                                const hasOffer = !!matchingOffer;
                                const savings = hasOffer
                                  ? matchingOffer.originalPrice -
                                    matchingOffer.offerPrice
                                  : 0;
                                const savingsPercentage = hasOffer
                                  ? Math.round(
                                      (savings / matchingOffer.originalPrice) *
                                        100
                                    )
                                  : 0;
                                ...
                                >
                                  <div className="space-y-2">
                                    <div className="flex justify-between items-center">
                                      <span className="font-semibold">
                                        {t("providers.orderNumber")} {order.id}
                                      </span>
                                      {order.status && (
                                        <span
          ...
          >
            <DialogContent className="sm:max-w-[500px]">
              <DialogHeader>
                <DialogTitle>
                  {t("providers.approveJoinRequestTitle")}
                </DialogTitle>
                <DialogDescription>
              ...
              <div className="space-y-4">
                <div>
                  <Label htmlFor="approveNotes" className="text-sm font-medium">
                    {t("providers.adminNotesOptional")}
                  </Label>
                  <Textarea
                    id="approveNotes"
                    value={approveNotes}
          ...
          >
            <DialogContent className="sm:max-w-[500px]">
              <DialogHeader>
                <DialogTitle>
                  {t("providers.rejectJoinRequestTitle")}
                </DialogTitle>
                <DialogDescription>
              ...
              <div className="space-y-4">
                <div>
                  <Label htmlFor="rejectReason" className="text-sm font-medium">
                    {t("providers.rejectionReasonRequired")}
                  </Label>
                  <Textarea
                    id="rejectReason"
                    value={rejectReason}
            ...
            provider={
              selectedProvider
                ? {
                    ...
                    officialDocuments:
                      selectedProvider.officialDocuments || undefined,
                  }
                : null
            }
          ...
          >
            <DialogContent className="sm:max-w-[600px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  {t("providers.detailedIncomeBreakdown").replace(
                    "{name}",
                    selectedProvider?.name || ""
                  )}
                </DialogDescription>
                          ...
                          <div className="p-4 bg-orange-50 rounded-lg">
                            <div className="flex items-center justify-between">
                              <span className="text-sm font-medium text-orange-700">
                                {t("providers.totalCommission")}
                              </span>
                              <span className="text-lg font-bold text-orange-800">
                                {renderCurrency(income.totalCommission)}
                              </span>
                            </div>
                          </div>
                          <div className="p-4 bg-purple-50 rounded-lg">
                            <div className="flex items-center justify-between">
                              <span className="text-sm font-medium text-purple-700">
                                {t("providers.netIncome")}
                              </span>
                              <span className="text-lg font-bold text-purple-800">
                                {renderCurrency(income.netIncome)}
                              </span>
                            </div>
                          </div>
          ...
          >
            <DialogContent className="sm:max-w-[800px] max-h-[85vh] overflow-hidden ">
              <DialogHeader className="rtl:text-right">
                ...
                <DialogDescription>
                  {t("providers.allServicesCategories").replace(
                    "{name}",
                    selectedProvider?.name || ""
                  )}
                </DialogDescription>
                ...
                <div className="space-y-6 overflow-y-auto max-h-[calc(80vh-140px)] pr-2">
                  ...
                  selectedProvider.providerServices.length > 0 ? (
                      ...
                      {(() => {
                        const groupedServices =
                          selectedProvider.providerServices.reduce(
                            ...
                            {} as Record<
                              string,
                              typeof selectedProvider.providerServices
                            >
                          );
                            ...
                            >
                              ...
                              <div className="grid gap-3">
                                {services.map((ps, index) => {
                                  ...
                                  const offer = selectedProvider?.offers?.find(
                                    (o) => o.originalPrice === ps.price
                                  );

                                  // Calculate commission amount
                                  const commissionAmount =
                                    ps.service?.commission || 0;
                                  const totalWithCommission =
                                    ps.price - commissionAmount;
                    ...
                    <div className="text-center py-8">
                      ...
                      <h3 className="text-lg font-semibold text-gray-900 mb-2">
                        {t("providers.noServicesAvailable")}
                      </h3>
                      <p className="text-muted-foreground">
