...
const transformInvoiceResponse = (apiInvoice: any): Invoice => {
  ...
  const totalCommission =
    apiInvoice.order?.servicesBreakdown?.reduce(
      (sum: number, service: ServiceBreakdown) =>
        sum + (service.commissionAmount || 0),
      0
    ) || 0;
    ...
    order: apiInvoice.order
      ? {
          ...
          service: {
            ...
            titleAr:
              apiInvoice.order.service?.titleAr ||
              apiInvoice.order.servicesBreakdown?.[0]?.serviceTitleAr ||
              "",
            titleEn:
              apiInvoice.order.service?.titleEn ||
              apiInvoice.order.servicesBreakdown?.[0]?.serviceTitleEn ||
              "",
            descriptionAr:
              apiInvoice.order.service?.descriptionAr ||
              apiInvoice.order.servicesBreakdown?.[0]?.serviceDescriptionAr ||
              "",
            descriptionEn:
              apiInvoice.order.service?.descriptionEn ||
              apiInvoice.order.servicesBreakdown?.[0]?.serviceDescriptionEn ||
              "",
...
export class InvoiceService {
  ...
  static async updatePaymentStatus(
    ...
    try {
      ...
      const response = await api.put(
        `/admin/invoices/${id}/payment-status`,
        body
      );
      return response.data;
    } catch (error) {
  ...
  static async markAsPaid(
    id: number,
    paymentMethod: string = "Admin Payment"
  ): Promise<Invoice> {
    try {
  ...
  static async getInvoiceStats(): Promise<InvoiceStats> {
    try {
      ...
      const unpaid = transformedInvoices.filter(
        (invoice: Invoice) => invoice.paymentStatus === "unpaid"
      ).length;
      const paid = transformedInvoices.filter(
        (invoice: Invoice) => invoice.paymentStatus === "paid"
      ).length;
      const failed = transformedInvoices.filter(
        (invoice: Invoice) => invoice.paymentStatus === "failed"
      ).length;
      ...
      transformedInvoices.forEach((invoice: Invoice) => {
        ...
        const providerAmount =
          invoice.order?.servicesBreakdown?.reduce(
            (sum: number, service: ServiceBreakdown) =>
